...
import { parse } from './compiler-utils.js';
import { stringifyRequest } from './utils.js';
...
export default function load(buffer, ctx) {
	...
	if (descriptor.template) {
		let src = `./${filename}`;
		let req = stringifyRequest(`${src}?vue&type=template`);
	...
	if (descriptor.script) {
		let src = `./${filename}`;
		let req = stringifyRequest(`${src}?vue&type=script`);
		scriptImport = [
